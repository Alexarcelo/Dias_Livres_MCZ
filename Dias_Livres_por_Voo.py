...
def plotar_tabela_servicos_no_voo(df_in_out, selected_rows, df_final):
    ...
    df_ref = df_in_out[df_in_out['Reserva Mae'].isin(reserva_mae_unica)].groupby(['Servico', 'Modo do Servico']).agg({'Total Paxs': 'sum', 'Dias Livres': 'sum'}).reset_index()
...
def plotar_tabela_row_todos_servicos(df_ref_2, row1):
    ...
    with row1[0]:
        ...
        container_dataframe.dataframe(df_ref_2[['Reserva Mae', 'Servico', 'Voo IN', 'Data IN', 'Data OUT', 'Total Paxs', 'Qtd. Servicos', 'Dias Estadia', 'Dias Livres']]\
                                      .sort_values(by='Voo IN'), hide_index=True, use_container_width=True)     
...
st.session_state.aba_sheet = 'Configurações Maceió'

st.session_state.titulo = 'Dias Livres por Voo - Maceió'

st.session_state.titulo_2 = 'Dias Livres por Hotel - Maceió'
...
if len(st.session_state.df_final)>0:
    ...
    if selected_rows is not None and len(selected_rows)>0:
        ...
        selected_rows_2 = plotar_tabela_servicos_no_voo(st.session_state.df_final, selected_rows, df_final)
        ...
        if selected_rows_2 is not None and len(selected_rows_2)>0:
            ...
            df_ref_2 = st.session_state.df_final[(st.session_state.df_final['Voo IN'].isin(selected_rows['Voo IN'].unique().tolist())) & 
                                        (st.session_state.df_final['Servico'].isin(selected_rows_2['Servico'].unique().tolist()))].reset_index(drop=True)
            
            plotar_tabela_row_todos_servicos(df_ref_2, row2)
